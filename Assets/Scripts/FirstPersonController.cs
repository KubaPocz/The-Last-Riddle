...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (isGrounded)
        {
            ...
            velocity.y = 0; // Keep vertical movement stable
    ...
    private void LateUpdate()
    {
        if (cameraCanMove)
        {
            // Pobieramy wartości myszy
            float mouseX = Input.GetAxis("Mouse X");
            float mouseY = Input.GetAxis("Mouse Y");

            // USTAWIENIE STAŁEJ CZUŁOŚCI – bez dodatkowych korekcji

            yaw += mouseX * mouseSensitivity;
            pitch -= (invertCamera ? -1 : 1) * mouseY * mouseSensitivity;

            // Ograniczenie kąta patrzenia góra/dół
            pitch = Mathf.Clamp(pitch, -maxLookAngle, maxLookAngle);

            // Ustawienie rotacji gracza (obrót wokół Y)
            transform.rotation = Quaternion.Euler(0f, yaw, 0f);

            // Ustawienie rotacji kamery (obrót góra/dół)
    ...
    private void CheckGround()
    {
        ...
        float distance = 0.2f;
        isGrounded = Physics.Raycast(origin, Vector3.down, distance, LayerMask.GetMask("Default"));
    }
    private void HandleStepOffset()
    {
        ...
        foreach (Vector3 direction in directions)
        {
            ...
            if (Physics.Raycast(origin, checkDirection, out RaycastHit hitLower, 0.5f))
            {
